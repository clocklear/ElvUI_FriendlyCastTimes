...
function NP:Castbar_CustomDelayText(duration)
	if self.channeling then
		if self.channelTimeFormat == 'CURRENT' then
			self.Time:SetFormattedText('%s |cffaf5050%.1f|r', NP:TimeFormat(abs(duration - self.max)), self.delay)
		elseif self.channelTimeFormat == 'CURRENTMAX' then
			self.Time:SetFormattedText('%s / %s |cffaf5050%.1f|r', NP:TimeFormat(duration), NP:TimeFormat(self.max), self.delay)
		elseif self.channelTimeFormat == 'REMAINING' then
			self.Time:SetFormattedText('%s |cffaf5050%.1f|r', NP:TimeFormat(duration), self.delay)
		elseif self.channelTimeFormat == 'REMAININGMAX' then
			self.Time:SetFormattedText('%s / %s |cffaf5050%.1f|r', NP:TimeFormat(abs(duration - self.max)), NP:TimeFormat(self.max), self.delay)
		end
	else
		if self.castTimeFormat == 'CURRENT' then
			self.Time:SetFormattedText('%s |cffaf5050%s %.1f|r', NP:TimeFormat(duration), '+', self.delay)
		elseif self.castTimeFormat == 'CURRENTMAX' then
			self.Time:SetFormattedText('%s / %s |cffaf5050%s %.1f|r', NP:TimeFormat(duration), NP:TimeFormat(self.max), '+', self.delay)
		elseif self.castTimeFormat == 'REMAINING' then
			self.Time:SetFormattedText('%s |cffaf5050%s %.1f|r', NP:TimeFormat(abs(duration - self.max)), '+', self.delay)
		elseif self.castTimeFormat == 'REMAININGMAX' then
			self.Time:SetFormattedText('%s / %s |cffaf5050%s %.1f|r', NP:TimeFormat(abs(duration - self.max)), NP:TimeFormat(self.max), '+', self.delay)
		end
	end
end
...
function NP:Castbar_CustomTimeText(duration)
	if self.channeling then
		if self.channelTimeFormat == 'CURRENT' then
			self.Time:SetFormattedText('%s', NP:TimeFormat(abs(duration - self.max)))
		elseif self.channelTimeFormat == 'CURRENTMAX' then
			self.Time:SetFormattedText('%s / %s', NP:TimeFormat(abs(duration - self.max)), NP:TimeFormat(self.max))
		elseif self.channelTimeFormat == 'REMAINING' then
			self.Time:SetFormattedText('%s', NP:TimeFormat(duration))
		elseif self.channelTimeFormat == 'REMAININGMAX' then
			self.Time:SetFormattedText('%s / %s', NP:TimeFormat(duration), NP:TimeFormat(self.max))
		end
	else
		if self.castTimeFormat == 'CURRENT' then
			self.Time:SetFormattedText('%s', NP:TimeFormat(duration))
